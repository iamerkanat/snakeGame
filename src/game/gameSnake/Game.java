...
public class Game implements KeyListener {
    ...
    public Game() {
        ...
        window.setSize(width * dimention + 2 , height * dimention + 4);
    ...
    private boolean check_wall_collision(){
        if(player.getX() < 0 || player.getX() >= width * dimention || player.getY() < 0 || player.getY() >= height * dimention){
            return true;}
        return false;
    }

    private boolean check_food_collision() {
        if(player.getX() == food.getX() * dimention && player.getY() == food.getY() * dimention){
            return true;
        }
        return false;
    }
    ...
    @Override
    public void keyPressed(KeyEvent e) { //changes direction
    ...
    } //every time when you press key button, game will call this function
